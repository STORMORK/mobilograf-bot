...
`api/analytics.js` (POST) перевіряє Telegram initData на сервері й
...
Один файл, два методи — той самий підхід, що вже використовує
`api/admin-busy-dates.js` (GET+POST в одному файлі):

| Endpoint | Призначення |
|---|---|
| `api/analytics.js` (POST) | **публічний**, без авторизації — приймає одну подію від сайту; адмінів мовчки ігнорує |
| `api/analytics.js` (GET) | адмін-only: агрегована статистика для розділу «📊 Статистика» |

Доки таблиця не створена, POST просто нічого не зберігає (сайт
продовжує працювати як завжди), а GET повертає нульову статистику —
той самий підхід, що й у розділах вище.

## Telegram-бот: /start та кнопка запуску Mini App

Обробляється тим самим `api/send-message.js`, що вже приймає POST від
форми «Замовити зйомку» (не окремим endpoint-ом — цей проєкт вже впирався
в ліміт Vercel на кількість serverless-функцій, див. нижче). Кожен
Update, який надсилає Telegram, завжди містить поле `update_id` —
booking-payload (`name`/`phone`/`date`/`service`/`comment`) його ніколи
не має, тож `handler()` розпізнає й розводить два випадки в самому
початку, до будь-яких booking-специфічних перевірок (rate limit,
Content-Type, розмір тіла) - вони не повинні застосовуватись до
запитів від самого Telegram. На `/start` бот відповідає привітальним
текстом з inline-кнопкою «📱 Відкрити застосунок» (Web App-кнопка,
відкриває Mini App напряму в чаті), а також оновлює постійну кнопку меню
(зліва від поля вводу повідомлення) з тим самим написом і посиланням —
так користувачу не треба здогадуватись, куди тиснути.

Потребує одноразового ручного налаштування — Telegram сам не дізнається
про цей URL, поки йому не сказати:

### 1. Прив'язати webhook (один раз, через `BOT_TOKEN`)

```
https://api.telegram.org/bot<BOT_TOKEN>/setWebhook?url=https://mobilograf-bot.vercel.app/api/send-message.js
```

Відкрийте це посилання в браузері (підставивши свій `BOT_TOKEN` — той
самий, що вже заданий у Vercel Environment Variables) або виконайте як
`curl`. Успішна відповідь — `{"ok":true,"result":true,...}`.

### 2. (Опційно, рекомендовано) Захистити webhook секретом

Без цього кроку endpoint технічно приймає запити з будь-якою формою
тіла — секрет підтверджує, що запит справді від Telegram, а не від
когось, хто просто вгадав URL:

1. Задайте в Vercel Environment Variables (Production) новий рядок
   `TELEGRAM_WEBHOOK_SECRET` з довільним випадковим значенням.
2. Повторіть виклик `setWebhook` вище, додавши
   `&secret_token=<те саме значення>` в кінець URL.

### 3. Кнопка меню з'явиться одразу після першого `/start`

`setChatMenuButton` викликається кодом автоматично при кожному `/start`
(без `chat_id` — це змінює кнопку за замовчуванням для всіх
користувачів одразу, не лише для того, хто написав). Щоб кнопка
з'явилась миттєво, без очікування на чиєсь `/start`, можна одноразово
виконати той самий виклик вручну:

```
https://api.telegram.org/bot<BOT_TOKEN>/setChatMenuButton?menu_button={"type":"web_app","text":"📱 Відкрити застосунок","web_app":{"url":"https://mobilograf-bot.vercel.app/"}}
```

(URL-кодування фігурних дужок браузер зазвичай робить сам; якщо ні —
використайте `curl --data-urlencode` замість підстановки в адресний
рядок.)
