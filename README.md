...
`api/telegram-webhook.js` — єдиний endpoint у цьому репозиторії, що
приймає вхідні повідомлення від Telegram (не від Mini App), і відповідає
на `/start` привітальним текстом з inline-кнопкою «📱 Відкрити
застосунок» (Web App-кнопка, відкриває Mini App напряму в чаті), а
також підтримує постійну кнопку меню (зліва від поля вводу
повідомлення) з тим самим написом і посиланням — так користувачу не
треба здогадуватись, куди тиснути.
...
```
https://api.telegram.org/bot<BOT_TOKEN>/setWebhook?url=https://mobilograf-bot.vercel.app/api/telegram-webhook.js
```
